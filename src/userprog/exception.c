...
page_fault (struct intr_frame *f) 
{
  ...
  if(open_spot == -1 && p != NULL){
    ...
    struct frame* fr = frame_table[eviction_spot];
    struct sup_page *old_sup_page = fr->resident;
    int i;
    void*to_write;
    int swap_slot = get_open_swap_slot(swap_spots);
